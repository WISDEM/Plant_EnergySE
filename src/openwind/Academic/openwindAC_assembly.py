...
''' Run openWind from openMDAO 
    ...
    2014 09 22: conversion to Fused-Wind interface, added error checking
...
class openwindAC_assembly(BaseAEPModel): # todo: has to be assembly or manipulation and passthrough of aep in execute doesnt work
    ...
    def __init__(self, openwind_executable, workbook_path, 
                 ...
                 start_once=False,
                 debug=False):
        """ Creates a new BaseAEPModel Assembly object for OpenWind Academic """
        ...
        self.turb = ExtendedWindTurbinePowerCurveVT() # 14 09 23
    ...
    def execute(self):
        ...
        sys.stderr.write("Initialized trb from {:} - why?\n".format(owtg_file))
    ...
    def updateRptPath(self, rptPathName, newScriptName):
        ...
        if not hasattr(self,'ow'):
            sys.stderr.write("{:}:updateRptPath() 'ow' not defined yet\n".format(self.__class__))
...
def example():
    ...
    script_file = test_path + 'owacScript.xml' # optimize operation
    if modify_turbine:
        script_file = test_path + 'rtopScript.xml' # replace turbine, optimize
        if debug:
    ...
    #owAsy.configure() # added 14 09 23
    ...
    if True:
        ...
        owa.updateRptPath('newReport.txt', 'newTestScript.xml')
        if debug:
