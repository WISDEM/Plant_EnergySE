...
'''
  ...
  2014 09 22: conversion to Fused-Wind interface
...
from openmdao.lib.datatypes.api import Float, Int, VarTree, List
...
# 2014 09 22: fused_plant_vt will go away, to be replaced by vt.py
...
#@implement_base(BaseAEPAggregator) uncomment this when fused-wind can do proper argument checking
class OWACcomp(Component):
    ...
    # outputs (gross_aep and net_aep are required by BaseAEPAggregator)
...
if __name__ == "__main__":
    ...
    test_path = '../test/' # all test scripts should be located here (relative to this source file)
    ...
    owXMLname = test_path + 'rtecScript.xml' # replace turb, energy capture
    owXMLname = test_path + 'owacScript.xml' # optimize operation
    #owXMLname = test_path + 'rtopScript.xml' # replace turb, optimize
    ...
    base_turbine_file = test_path + 'Alstom6MW.owtg'
    base_turbine = turbfuncs.owtg_to_wtpc(base_turbine_file)
    if base_turbine is None:
        sys.stderr.write('\n*** ERROR - bad turbine file\n')
        exit()
    ...
    #wt_list = List([wt_list_elem for i in range(len(wt_positions)) ])
    
    # debug messages
    #print
    #print 'wtlelem   ', wt_list_elem.__class__
    #print 'wt_list   ', wt_list.__class__
    #for e in wt_list:
    #    print '  ', e.__class__
    #print 'ow.wtlo  ', ow.wt_layout.__class__
    #print 'ow.wtlowl ', ow.wt_layout.wt_list.__class__
    #print 'ow.wtlown ', ow.wt_layout.wt_names.__class__
    #print 'ow.wtlowp ', ow.wt_layout.wt_positions.__class__
    #print
    # end debug 
    ...
    if start_once:
        if debug:
            sys.stderr.write('\n*** INFO: terminating openWind\n')
        ow.terminateOW()
            
    else:
        sys.stderr.write('\n*** INFO: terminating openWind even though start_once is False\n')
        ow.terminateOW()
